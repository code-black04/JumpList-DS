...
string JumpList::get(int i) const
{
    ...
    while (current != nullptr)
    { // If index is within the current node's gap range, use next pointers
...
void JumpList::deleteNode(Node *nodeToDelete, Node *previous, Node *previousJump)
{
    ...
    else if (nodeToDelete->gap_ != 0)
    { // Update jump pointers to bypass the deleted node
...
void JumpList::findClosestNodes(const string &s, Node *&current, Node *&previous, Node *&previousJump)
{
           ...
           (current->next_ != nullptr && current->next_->data_ <= s))
    {
        ...
        else if (current->next_ != nullptr && current->next_->data_ <= s)
        { // If the current node is a jump node (gap > 0), update `previousJump`
            if (current->gap_ != 0)
            {
                previousJump = current;
            }
